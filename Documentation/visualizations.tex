...
\begin{figure}
	\centering
	\includegraphics[scale=0.8]{treemap_example.png}
...
\INITIAL{L}{ooking at the scenarios} presented in the previous section, it becomes clear that the vast majority of visualization scenarios which would be encountered during exploratory or in-situ processing can be handled using very basic tools. In Table \ref{tbl:formats} a visual summary is given of the areas in which each of the discussed visualizations is most appropriate. In cases with limited applicability, it is of course important to keep in mind that more complex data formats can typically be reduced so that their features fit well into the more easily visualized data formats.
